...
const User = () => {
  ...
  const fetchElements = useCallback(async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/elements');
      setElements(response.data.elements);
      console.log('Fetched elements:', response.data.elements); 
    } catch (error) {
      console.error('Error fetching elements:', error);
      Alert.alert('Error', 'Unable to fetch elements. Please try again later.');
    } finally {
      setIsLoading(false);
      setRefreshing(false);
    }
